...
function detectRegressions(db: SentinelDB): { status: string; alerts: Array<{ metric: string; severity: string; message: string }> } {
  try {
...
export class MarkdownReport {
  ...
  generate(options: { days?: number } = {}): string {
    ...
    // --- Footer ---
    lines.push('---');
    lines.push(`*Generated by Agent Sentinel on ${new Date().toISOString().substring(0, 10)}*`);
