...
export class MetricsAnalyzer {
  ...
  // -----------------------------------------------------------------------
  private computeThinkingDepthMedian(db: SentinelDB, raw: Database.Database): void {
  ...
  // -----------------------------------------------------------------------
  private computeThinkingDepthRedactedPct(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
  ...
  // 3. read_edit_ratio
  // -----------------------------------------------------------------------
  private computeReadEditRatio(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // -----------------------------------------------------------------------
  private computeResearchMutationRatio(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // 5. blind_edit_rate
  // -----------------------------------------------------------------------
  private computeBlindEditRate(db: SentinelDB, raw: Database.Database): void {
  ...
  // 6. write_vs_edit_pct
  // -----------------------------------------------------------------------
  private computeWriteVsEditPct(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // -----------------------------------------------------------------------
  private computeFirstToolReadPct(db: SentinelDB, raw: Database.Database): void {
  ...
  // -----------------------------------------------------------------------
  private computeReasoningLoopsPer1k(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
  ...
  // 9. laziness_total
  // -----------------------------------------------------------------------
  private computeLazinessTotal(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
  ...
  // -----------------------------------------------------------------------
  private computeSelfAdmittedFailuresPer1k(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
  ...
  // -----------------------------------------------------------------------
  private computeUserInterruptsPer1k(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
  ...
  // 12. sentiment_ratio
  // -----------------------------------------------------------------------
  private computeSentimentRatio(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // 13. frustration_rate
  // -----------------------------------------------------------------------
  private computeFrustrationRate(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
             COUNT(*) AS total,
  ...
  // -----------------------------------------------------------------------
  private computeSessionAutonomyMedian(db: SentinelDB, raw: Database.Database): void {
  ...
  // 15. edit_churn_rate
  // -----------------------------------------------------------------------
  private computeEditChurnRate(db: SentinelDB, raw: Database.Database): void {
  ...
  // -----------------------------------------------------------------------
  private computeBashSuccessRate(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // 17. subagent_pct
  // -----------------------------------------------------------------------
  private computeSubagentPct(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // 18. context_pressure
  // -----------------------------------------------------------------------
  private computeContextPressure(db: SentinelDB, raw: Database.Database): void {
  ...
  // 19. cost_estimate
  // -----------------------------------------------------------------------
  private computeCostEstimate(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // -----------------------------------------------------------------------
  private computePromptsPerSession(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
  ...
  // -----------------------------------------------------------------------
  private computeTimeOfDayQuality(db: SentinelDB, raw: Database.Database): void {
  ...
  // -----------------------------------------------------------------------
  private computeToolDiversity(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
      SELECT ${dateExpr} AS day,
  ...
  // -----------------------------------------------------------------------
  private computeTokenEfficiency(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
  ...
  // -----------------------------------------------------------------------
  private computeSessionLength(db: SentinelDB, raw: Database.Database): void {
    const rows = raw.prepare(`
