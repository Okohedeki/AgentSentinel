...
function getDefaultDbPath(): string {
  const dir = path.join(os.homedir(), '.agent-sentinel');
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  return path.join(dir, 'sentinel.db');
}
...
export class SentinelDB {
  ...
  // ---------------------------------------------------------------------------
  // Context Bus: File Operations
  // ---------------------------------------------------------------------------

  insertFileOperation(op: {
    session_id: string; project_path: string; file_path: string;
    operation: string; timestamp: string; agent_id: string;
    reason?: string; before_hash?: string; after_hash?: string;
  }): void {
    this.db.prepare(`
      INSERT INTO ctx_file_operations (session_id, project_path, file_path, operation, timestamp, agent_id, reason, before_hash, after_hash)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(op.session_id, op.project_path, op.file_path, op.operation, op.timestamp,
      op.agent_id, op.reason || null, op.before_hash || null, op.after_hash || null);
  }

  getRecentFileOperations(sessionId: string, limit: number = 200): any[] {
    return this.db.prepare(`
      SELECT * FROM ctx_file_operations WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?
    `).all(sessionId, limit);
  }

  // ---------------------------------------------------------------------------
  // Context Bus: Tool Call Cache
  // ---------------------------------------------------------------------------

  insertToolCacheEntry(tc: {
    session_id: string; tool_name: string; input_json: string;
    output_json?: string; timestamp: string; ttl_seconds: number; input_hash: string;
  }): void {
    this.db.prepare(`
      INSERT INTO ctx_tool_cache (session_id, tool_name, input_json, output_json, timestamp, ttl_seconds, input_hash)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(tc.session_id, tc.tool_name, tc.input_json, tc.output_json || null,
      tc.timestamp, tc.ttl_seconds, tc.input_hash);
  }

  lookupToolCache(sessionId: string, inputHash: string): unknown | null {
    const row = this.db.prepare(`
      SELECT output_json, timestamp, ttl_seconds FROM ctx_tool_cache
      WHERE session_id = ? AND input_hash = ?
      ORDER BY timestamp DESC LIMIT 1
    `).get(sessionId, inputHash) as any;
    ...
    const now = Date.now();
    if (now - entryTime > row.ttl_seconds * 1000) return null;
  ...
  // ---------------------------------------------------------------------------
  // Context Bus: Decisions
  // ---------------------------------------------------------------------------

  insertDecision(d: {
    session_id: string; description: string; rationale: string;
    timestamp: string; agent_id: string; reversible: boolean;
  }): void {
    this.db.prepare(`
      INSERT INTO ctx_decisions (session_id, description, rationale, timestamp, agent_id, reversible)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(d.session_id, d.description, d.rationale, d.timestamp, d.agent_id, d.reversible ? 1 : 0);
  }

  getRecentDecisions(sessionId: string, limit: number = 50): any[] {
    return this.db.prepare(`
      SELECT * FROM ctx_decisions WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?
    `).all(sessionId, limit);
  }

  // ---------------------------------------------------------------------------
  // Context Bus: Task State
  // ---------------------------------------------------------------------------

  upsertTaskState(ts: {
    session_id: string; project_path: string; task_description?: string;
    started_at?: string; last_activity?: string; completed_steps?: string;
    remaining_steps?: string; blockers?: string;
  }): void {
    this.db.prepare(`
      INSERT INTO ctx_task_state (session_id, project_path, task_description, started_at, last_activity, completed_steps, remaining_steps, blockers)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(session_id) DO UPDATE SET
        ...
        completed_steps = COALESCE(excluded.completed_steps, completed_steps),
        remaining_steps = COALESCE(excluded.remaining_steps, remaining_steps),
        blockers = COALESCE(excluded.blockers, blockers)
    `).run(ts.session_id, ts.project_path, ts.task_description || null,
      ts.started_at || null, ts.last_activity || null,
      ts.completed_steps || null, ts.remaining_steps || null, ts.blockers || null);
  }

  getTaskState(sessionId: string): any | null {
    return this.db.prepare(`
      SELECT * FROM ctx_task_state WHERE session_id = ?
    `).get(sessionId) || null;
  }

  // ---------------------------------------------------------------------------
  // Context Bus: Agent History
  // ---------------------------------------------------------------------------

  insertAgentRun(run: {
    session_id: string; agent_type: string; model_version?: string;
    start_time: string; end_time?: string; quality_score: number;
    handoff_reason?: string; lever?: string;
  }): void {
    this.db.prepare(`
      INSERT INTO ctx_agent_history (session_id, agent_type, model_version, start_time, end_time, quality_score, handoff_reason, lever)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(run.session_id, run.agent_type, run.model_version || null,
      run.start_time, run.end_time || null, run.quality_score,
      run.handoff_reason || null, run.lever || null);
  }

  getAgentHistory(sessionId: string): any[] {
    return this.db.prepare(`
      SELECT * FROM ctx_agent_history WHERE session_id = ? ORDER BY start_time ASC
    `).all(sessionId);
  }

  // ---------------------------------------------------------------------------
  // Context Bus: Watcher Events
  // ---------------------------------------------------------------------------

  insertWatcherEvent(evt: {
    session_id: string; severity: string; failure_mode: string;
    metrics_json: string; timestamp: string;
  }): void {
    this.db.prepare(`
      INSERT INTO watcher_events (session_id, severity, failure_mode, metrics_json, timestamp)
      VALUES (?, ?, ?, ?, ?)
    `).run(evt.session_id, evt.severity, evt.failure_mode, evt.metrics_json, evt.timestamp);
  }

  getRecentWatcherEvents(limit: number = 50): any[] {
    return this.db.prepare(`
      SELECT * FROM watcher_events ORDER BY timestamp DESC LIMIT ?
    `).all(limit);
  }
  ...
  clearContextBus(sessionId: string): void {
    const tables = ['ctx_file_operations', 'ctx_tool_cache', 'ctx_decisions', 'ctx_task_state', 'ctx_agent_history'];
